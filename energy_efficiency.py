...
def format_output(data):
    y1 = data.pop('heating_loan')
    y1 = np.array(y1)
    y2 = data.pop('cooling_loan')
    y2 = np.array(y2)
    return y1, y2
...
df.columns = ['rel_compactness', 'surface_area', 'wall_area', 'roof_area', 'overall_height', 'orientation', 'glazing_area', 'glazing_area_dist', 'heating_loan', 'cooling_loan']
df
...
train_stats.pop('heating_loan')
train_stats.pop('cooling_loan')
...
def final_model(input_shape, repetitions):
    ...
    heating_loan = Dense(units='1', name='heating_loan')(x)
    cooling_loan = Dense(units='1', name='cooling_loan')(x)

    model = Model(inputs=input_, outputs=[heating_loan, cooling_loan])
...
model.compile(optimizer=rms,
            loss = {'heating_loan' : 'mean_squared_error',
                  'cooling_loan' : 'mean_squared_error'
                 },
            metrics = {'heating_loan' : tf.keras.metrics.RootMeanSquaredError(),
                     'cooling_loan': tf.keras.metrics.RootMeanSquaredError()
                   }
            )
...
plot_diff(test_Y[0], predictions[0], title='Heating Loan')
plot_diff(test_Y[1], predictions[1], title='Cooling Loan')
